class FacturaPdf < Prawn::Document
  ...
  def header
    ...
    # cell_1 = make_cell(:content => "#{@operation[0].kind_invoice} ##{@invoice[0].id}")
    # cell_2 = make_cell(:content => "")
    
    y_position = cursor - 0
    image "#{Rails.root}/app/assets/images/logo.png", :scale => 0.75
    ...
    text "#{@operation[0].operacion} ##{@operation[0].id}", :color =>  rgb='f22415', :size => 30, :style => :bold
    ...
    bounding_box([0, y_position], :width => 400, :height => 200) do
      ...
      text "#{@operation[0].client.full_name}"
      text "ID:", size: 14, style: :bold
      text "#{@operation[0].client.identificacion}"
      text "Address:", size: 14, style: :bold
      text "#{@operation[0].client.direccion}"    
  ...
  def table_content
    ...
    table invoice_rows do
      ...
      self.column_widths = [100, 40, 80, 120, 80, 120]
    end
  end
 
  def invoice_rows
    [['Product', 'Qty', 'Price','Subtotal', 'Tax', 'Total']] +
      @operation.map do |operation|
      [
        operation.product.producto, 
        operation.cantidad, 
        operation.product.precio,
        operation.subtotal,
        operation.impuestos,
        operation.total
      ]
